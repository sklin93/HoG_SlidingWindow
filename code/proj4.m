...
%test_scn_path = fullfile(data_path,'test_scenes/test_jpg'); %CMU+MIT test scenes
 test_scn_path = fullfile(data_path,'live_demo_test/data/test_jpg'); %Bonus scenes
label_path = fullfile(data_path,'live_demo_test/data/ground_truth_bboxes.txt'); %the ground truth face locations in the test set
...
%     features_pos = get_positive_features( train_path_pos, feature_params );
%     fprintf('Extracting negative features...\n');
%     num_negative_examples = 85000; %Higher will work strictly better, but you should start with 10000 for debugging
%     features_neg = get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
%     save('features_pos.mat', 'features_pos');   
%     save('features_neg.mat', 'features_neg');
% else
%     fprintf('Loading positive and negative features...\n');
...
% if ~exist('original_w.mat', 'file')||~exist('original_b.mat', 'file')|| refresh
% [pos_size, ~] = size(features_pos);
% [neg_size, dimension] = size(features_neg);
% %initialize X, Y, lambda
% X = [features_pos; features_neg];
% X = X';
% Y = ones(pos_size + neg_size, 1);
% Y(pos_size+1 : end) = -1;
% lambda = 0.0001;
% fprintf('First round training...\n');
% [w,b] = vl_svmtrain(X, Y, lambda);
% save('original_w.mat', 'w');   
% save('original_b.mat', 'b');
% else
%     fprintf('Loading w and b by the original training round...\n');
%     load('original_w.mat','w');
%     load('original_b.mat','b');
% end
...
% confidence level.
if ~exist('retrained_w.mat', 'file')||~exist('retrained_b.mat', 'file')|| refresh
    [hard_negative_features, hard_negative_num] = get_hard_negatives( non_face_scn_path, w, b, feature_params );

    fprintf('re-train SVM with hard negatives...\n');
    X = [features_pos; features_neg; hard_negative_features];
    X = X';
    Y = ones(pos_size + neg_size + hard_negative_num, 1);
    Y(pos_size+1 : end) = -1;
    lambda = 0.0001;

    [w,b] = vl_svmtrain(X, Y, lambda);
    save('retrained_w.mat', 'w');   
    save('retrained_b.mat', 'b');
else
    fprintf('Loading w and b after mining hard negatives...\n');
    load('retrained_w.mat','w');
    load('retrained_b.mat','b');
end
...
fprintf('run detector on test set...\n');
